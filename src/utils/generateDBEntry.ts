...
import { fetchFromBlockfrost, posixToSlot, selectLucidWallet } from "./wallet.js";
...
async function generateDBEntry() {
    ...
    const startSlot = (await fetchFromBlockfrost(`txs/${appliedScripts.projectTokenHolder.initOutRef.txHash}`)).slot as number
    ...
    const entry: DynamoTTEntry = {
        ...
        lastCallDate: {
            M: {
                Slot: {
                    S: posixToSlot(Number(process.env.DEADLINE as string) - TWENTY_FOUR_HOURS_POSIX).toString()
                },
                SlotOffset: {
                    N: PREVIEW_OFFSET
                }
